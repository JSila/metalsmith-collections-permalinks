...
function plugin(options){
  ...
  return function(files, metalsmith, done){
    ...
    Object.keys(files).forEach(function(file){
      ...
      var collOptions = (data.collection in options) ? options[data.collection] : options['*'];
      var pattern = collOptions.pattern;
      var path = replace(pattern, data, collOptions) || resolve(file);
      var fam = family(file, files);


      if (collOptions.relative) {
      ...
      var out = join(path, collOptions.indexFile || 'index.html');
      delete files[file];
      files[out] = data;
    });

    // add duplicates for relative files after processing to avoid double-dipping
    // note: `dupes` will be empty if `collOptions.relative` is false
...
function normalize(options){
  ...
  for (var collection in options) {
      var collOptions = options[collection];
      options[collection].date = collOptions.date ? format(collOptions.date) : format('YYYY/MM/DD');
      options[collection].relative = collOptions.hasOwnProperty('relative') ? collOptions.relative : true;
  }
  return options;
}
